...
curl -fsSL https://www.mongodb.org/static/pgp/server-${MONGO_VERSION}.asc | sudo gpg --dearmor --yes -o /etc/apt/keyrings/mongodb-7.0.gpg
...
#mongosh --eval 'db.createUser({ user: "admin", pwd: "admin1234", roles: [{ role: "root", db: "admin" }] })
mongosh --eval 'db.createUser({ user: "admin", pwd: "admin1234", roles: [{ role: "root", db: "admin" }] })'
